...
export default function Dashboard({ session }: { session: Session }) {
  ...
  const handleUpdateOrderStatus = async (pedido: Pedido, novoStatus: string) => {
    try {
      ...
      console.log("okko")
      // ---- DISPARA O E-MAIL DE ATUALIZAÇÃO ----
      if (pedido.email_cliente && loja) {
        const payload = {
          tipo: 'status_update',
          dados_pedido: {
            id: pedido.id,
        ...
        supabase.functions.invoke('send-email', { body: payload }).catch(console.error);
      }
                  ...
                  <div key={pedido.id} className="border-b border-gray-700 pb-4">
                    ...
                    <div className="flex items-center gap-2 mt-2 sm:mt-0">
                        ...
                        <select
                          id={`status-${pedido.id}`}
                          value={pedido.status}
                          // Passa o objeto 'pedido' inteiro para a função
                          onChange={(e) => handleUpdateOrderStatus(pedido, e.target.value)}
